...
class CDbCommand extends CComponent
{
	...
	public function prepare()
	{
		if($this->_statement==null)
		{
			...
			catch(Exception $e)
			{
				...
				Yii::log('CDbCommand', 'preparing SQL', $this->getText(), YII_ERROR);
	...
	public function execute($params=array())
	{
		...
		try
		{
			Yii::log(__CLASS__, __FUNCTION__, 'system.db.CDbCommand.execute('.$this->getText().')' , YII_INFO);
		...
		catch(Exception $e)
		{
			...
			Yii::log('CDbCommand', 'execute()', $message.' sql:'.$this->getText().$par, YII_ERROR);
            if(YII_DEBUG)
	...
	private function queryInternal($method,$mode,$params=array())
	{
		...
		catch(Exception $e)
		{
			...
			Yii::log('CDbCommand', $method, $message.' sql:'.$this->getText().$par, YII_ERROR);
            if(YII_DEBUG)
            	$message .= '. The SQL statement executed was: '.$this->getText().$par;
			throw new CDbException(Yii::t('yii','CDbCommand failed to execute the SQL statement: {error}',
				array('{error}'=>$message)),(int)$e->getCode(),$errorInfo);
