...
abstract class EMongoDocument extends EMongoEmbeddedDocument
{
		...
		if($this->beforeSave())
		{
			...
			foreach($this->getValidators() as $validator){
				if($validator instanceof CNumberValidator){
					$intAttributes = $validator->attributes;
					break;
				}
			}
			foreach ($intAttributes as $attribute){
				$rawData[$attribute] = (int)$rawData[$attribute];
			}
	...
	public function update(array $attributes=null, $modify = false)
	{
		...
		if($this->beforeSave())
		{
			...
			foreach ($intAttributes as $attribute){
				$rawData[$attribute] = (int)$rawData[$attribute];
				if($attribute === '_id')$this->_id = (int)$this->_id;
			}
	...
	private function createPkCriteria($pk, $multiple=false)
	{
		...
		$criteria = new EMongoCriteria();
		$pkValidator = $this->getValidators($pkField);
		if($pkValidator[0]->integerOnly){
			$pk = (int)$pk;
		}
